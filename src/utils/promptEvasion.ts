...
export const TECHNIQUE_LABELS: Record<EvasionTechnique, string> = {
  // --- Invisible Characters (highest bypass) ---
  variationSelectors: "Variation Selectors (~95%)",
  zwsp: "Zero-Width Spaces (~90%)",
  zwj: "Zero-Width Joiners (~90%)",
  zwnj: "Zero-Width Non-Joiner (~90%)",
  wordJoiner: "Word Joiner U+2060 (~88%)",
  invisibleSeparator: "Invisible Separator U+2063 (~85%)",
  softHyphen: "Soft Hyphens (~85%)",
  // --- Homoglyphs (high bypass) ---
  cyrillic: "Cyrillic Homoglyphs (~80%)",
  greek: "Greek Homoglyphs (~78%)",
  smallCaps: "Small Caps ɴᴀᴋᴇᴅ (~75%)",
  superscript: "Superscript ⁿᵃᵏᵉᵈ (~70%)",
  // --- Unicode Math Variants (high bypass) ---
  mathItalic: "Math Italic 𝑛𝑎𝑘𝑒𝑑 (~75%)",
  mathScript: "Math Script 𝓃𝒶𝓀𝑒𝒹 (~75%)",
  mathBoldScript: "Math Bold Script 𝓷𝓪𝓴𝓮𝓭 (~73%)",
  mathFraktur: "Math Fraktur 𝔫𝔞𝔨𝔢𝔡 (~73%)",
  mathDoubleStruck: "Math Double-Struck 𝕟𝕒𝕜𝕖𝕕 (~72%)",
  mathBold: "Math Bold 𝐧𝐚𝐤𝐞𝐝 (~70%)",
  mathBoldItalic: "Math Bold Italic 𝒏𝒂𝒌𝒆𝒅 (~70%)",
  mathBoldFraktur: "Math Bold Fraktur 𝖓𝖆𝖐𝖊𝖉 (~70%)",
  mathSanSerif: "Math Sans-Serif (~68%)",
  mathSanBold: "Math Sans Bold (~68%)",
  mathSanItalic: "Math Sans Italic (~68%)",
  mathSanBoldItalic: "Math Sans Bold Italic (~68%)",
  mathMonospace: "Math Monospace 𝚗𝚊𝚔𝚎𝚍 (~65%)",
  // --- Encoding (medium-high, model dependent) ---
  rot13: "ROT13 anxrq (~70%)",
  base64: "Base64 bmFrZWQ= (~65%)",
  hex: "Hex 6e 61 6b 65 64 (~60%)",
  htmlEntities: "HTML Entities &#110;... (~60%)",
  urlEncoding: "URL Encoding %6E%61... (~55%)",
  morse: "Morse Code -. .- -.- (~50%)",
  binary: "Binary 01101110... (~40%)",
  // --- Advanced Unicode ---
  bidiOverride: "Bidi Override RTL (~70%)",
  zalgo: "Zalgo Text ṋ̸̢a̷̓k̵̈e̴̊d̵̈ (~65%)",
  whitespaceVariants: "Unicode Whitespace Mix (~60%)",
  fullwidth: "Fullwidth ｎａｋｅｄ (~55%)",
  enclosed: "Enclosed ⓝⓐⓚⓔⓓ (~50%)",
  negativeSquared: "Neg. Squared 🅽🅰🅺🅴🅳 (~50%)",
  upsideDown: "Upside Down pǝʞɐu (~45%)",
  // --- Linguistic (medium, context dependent) ---
  strategicMisspell: "Strategic Misspell (~60%)",
  vowelRemoval: "Vowel Removal nkd (~55%)",
  pigLatin: "Pig Latin akednay (~50%)",
  charDoubling: "Char Doubling nnakkedd (~45%)",
  // --- Separators & Structural (lower) ---
  diacritics: "Diacritics ńâk̈ěḑ (~50%)",
  hyphenated: "Hyphenated n-a-k-e-d (~40%)",
  underscored: "Underscored n_a_k_e_d (~40%)",
  dotSeparated: "Dot Separated n.a.k.e.d (~35%)",
  interleavedNoise: "Noise n*a#k!e@d (~35%)",
  spaceSeparated: "Space Separated n a k e d (~30%)",
  // --- Well-known (often caught) ---
  leetspeak: "Leetspeak n4k3d (~25%)",
  mixedCase: "MiXeD cAsE (~20%)",
  reversed: "Reversed dekan (~15%)",
  // --- Meta ---
  all: "All Variants",
};
