...
function ProviderBadge({ provider }: { provider: ProviderType }) {
  const providerName = provider === "gemini" ? "Gemini" : provider === "replicate" ? "Replicate" : provider === "kie" ? "Kie.ai" : provider === "wavespeed" ? "WaveSpeed" : provider === "comfyui" ? "ComfyUI" : provider === "xai" ? "xAI" : provider === "bfl" ? "BFL" : provider === "byteplus" ? "BytePlus" : "fal.ai";
  ...
  return (
    <span className="text-neutral-500 shrink-0" title={providerName}>
      {provider === "gemini" ? (
        ...
        <span className="w-4 h-4 flex items-center justify-center text-[10px] font-bold">BFL</span>
      ) : provider === "byteplus" ? (
        <span className="w-4 h-4 flex items-center justify-center text-[10px] font-bold">BP</span>
      ) : (
...
export function GenerateImageNode({ id, data, selected }: NodeProps<NanoBananaNodeType>) {
  ...
  const { replicateApiKey, falApiKey, kieApiKey, xaiApiKey, bflApiKey, byteplusApiKey, comfyuiServerUrl, replicateEnabled, kieEnabled, xaiEnabled, bflEnabled, byteplusEnabled, comfyuiEnabled } = useProviderApiKeys();
  ...
  const enabledProviders = useMemo(() => {
    ...
    // Add BFL if configured
    if (bflEnabled && bflApiKey) {
      providers.push({ id: "bfl", name: "BFL" });
    }
    // Add BytePlus if configured
    if (byteplusEnabled && byteplusApiKey) {
      providers.push({ id: "byteplus", name: "BytePlus" });
    }
    // Add ComfyUI if configured
    if (comfyuiEnabled && comfyuiServerUrl) {
      providers.push({ id: "comfyui", name: "ComfyUI" });
    }
    return providers;
  }, [replicateEnabled, replicateApiKey, kieEnabled, kieApiKey, xaiEnabled, xaiApiKey, bflEnabled, bflApiKey, byteplusEnabled, byteplusApiKey, comfyuiEnabled, comfyuiServerUrl]);
